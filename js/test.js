if (document.querySelector('.main-index')) {
  ...
  let servicesLinks = services.querySelectorAll('.services__item-description');
  let serviceDescriptions = services.querySelectorAll('.service-description__item ');
  ...
  for (let servicesLink of servicesLinks) {
      servicesLink.addEventListener('click', (evt) => {
        event.preventDefault();
        for (let servicesLink of servicesLinks) {
          servicesLink.classList.remove('services__item-description--current');
        }
        for (let servicesLink of servicesLinks) {
          if (servicesLink == this) {
            servicesLink.classList.add('services__item-description--current');
            for (serviceDescription of serviceDescriptions) {
              serviceDescription.classList.remove('service-description__item--current');
            }
            serviceDescription.classList.add('service-description__item--current');
          }
        }
      })
  }
